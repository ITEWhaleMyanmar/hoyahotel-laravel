...
@section('title', trans('admin.special.actions.index'))

@section('body')

    <special-listing
        :data="{{ $data->toJson() }}"
        :url="'{{ url('admin/specials') }}'"
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.special.actions.index') }}
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0" href="{{ url('admin/specials/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.special.actions.create') }}</a>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block">
                            ...
                            <table class="table table-hover table-listing">
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' :column="'id'">{{ trans('admin.special.columns.id') }}</th>
                                        <th is='sortable' :column="'title'">{{ trans('admin.special.columns.title') }}</th>
                                        <th is='sortable' class="text-center" :column="'published_at'">{{ trans('admin.special.columns.published_at') }}</th>
                                        <th is='sortable' :column="'enabled'">{{ trans('admin.special.columns.enabled') }}</th>

                                        <th></th>
                                    </tr>
                                    <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="6">
                                            <span class="align-middle font-weight-light text-dark">{{ trans('brackets/admin-ui::admin.listing.selected_items') }} @{{ clickedBulkItemsCount }}.  <a href="#" class="text-primary" @click="onBulkItemsClickedAll('/admin/specials')" v-if="(clickedBulkItemsCount < pagination.state.total)"> <i class="fa" :class="bulkCheckingAllLoader ? 'fa-spinner' : ''"></i> {{ trans('brackets/admin-ui::admin.listing.check_all_items') }} @{{ pagination.state.total }}</a> <span class="text-primary">|</span> <a
                                                        href="#" class="text-primary" @click="onBulkItemsClickedAllUncheck()">{{ trans('brackets/admin-ui::admin.listing.uncheck_all_items') }}</a>  </span>

                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3" @click="bulkDelete('/admin/specials/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                                    ...
                                    <td>@{{ item.id }}</td>
                                        <td>@{{ item.title }}</td>
                                            ...
                                            <span v-if="item.published_at <= now">
                                                ...
                                                <span v-if="item.published_at > now">
                                                <small>{{ trans('admin.special.actions.will_be_published') }}</small><br />
                                                @{{ item.published_at | datetime('DD.MM.YYYY, HH:mm') }}
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner" href="{{ url('admin/specials/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.special.actions.create') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
